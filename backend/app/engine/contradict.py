...
def _c1(text: str) -> list[Contradiction]:
    ...
    if len(vals) >= 2:
        _, m = seen[-1]
        return [
            _cite("C1", text, m, f"amount mismatch in one document: {sorted(vals)}")
        ]
...
def _c2(text: str) -> list[Contradiction]:
    ...
    m = re.search(
        r"security\s*deposit.{0,80}non[-\s]?refundable|non[-\s]?refundable.{0,80}deposit",
        text,
        FLAGS,
    )
    if m:
        out.append(
            _cite(
                "C2", text, m, "non-refundable deposit understates Sec-11 refund right"
            )
        )
    m = re.search(r"refund\w*.{0,80}(?:two|three|2|3)\s*months?", text, FLAGS)
    if m:
        out.append(
            _cite("C2", text, m, "refund slower than 1-month statutory timeline")
        )
    m = re.search(
        r"overstay.{0,80}(?:1\.5|one\s+and\s+a\s+half)\s*(?:times|x)", text, FLAGS
    )
    if m:
...
def _c3(text: str, findings: list[Finding]) -> list[Contradiction]:
    for f in findings:
        if (
            f.rule_id == "R05"
            and f.status == "hit"
            and f.data.get("notice_months") is not None
        ):
            ...
            if m:
                return [
                    _cite(
                        ...
                        text,
                        m,
                        f"revision notice {f.data['notice_months']}mo < 3mo",
                    )
                ]
    m = re.search(
        r"(?:notice\s+of\s+)?(?:(\d+)\s*days?|one\s+month).{0,40}(?:revis|increas).{0,20}rent",
        text,
        FLAGS,
    )
    if m:
...
def _c4(text: str) -> list[Contradiction]:
    m = re.search(
        r"(?:tds|194[-\s]?i\s?b).{0,40}5\s*%|5\s*%.{0,40}(?:tds|194[-\s]?i\s?b)",
        text,
        FLAGS,
    )
    if m:
