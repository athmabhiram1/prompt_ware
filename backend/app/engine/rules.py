...
RULES: list[RuleSpec] = [
    RuleSpec(
        "R01", "deposit", 4, 1, True, [r"security\s*deposit|deposit\s*(?:of|amount|is)"]
    ),
    RuleSpec(
        "R02", "deposit", 4, 1, True, [r"security\s*deposit|deposit\s*(?:of|amount|is)"]
    ),
    RuleSpec(
        "R03",
        "deposit",
...
def _hit(
    ...
    tier: int,
    note: str = "",
    data: dict[str, str | float | bool] | None = None,
) -> Finding:
...
def _absent(rule: RuleSpec, walkaway: bool) -> Finding:
    ...
    return Finding(
        ...
        status="missing" if walkaway else "pass",
        tier=tier,
        note="clause absent" + (" — walkaway" if walkaway else ""),
    )
...
def _scan_rule(text: str, rule: RuleSpec, kind: str) -> Finding:
    ...
    if rid == "R01" and kind != "residential":
        return Finding(
            ...
            status="pass",
            tier=1,
            note="n/a for non-residential premise",
        )
    if rid == "R02" and kind != "non-residential":
        return Finding(
            ...
            status="pass",
            tier=1,
            note="n/a for residential premise",
        )
    if rid in ("R01", "R02"):
    ...
    if rid == "R07":
        ...
        return Finding(
            ...
            status="pass",
            tier=2,
            note="no maintenance clause — no risk",
        )
    if rid == "R12":
