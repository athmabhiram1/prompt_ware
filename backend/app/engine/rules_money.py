...
def deposit_rule(text: str, rule: RuleSpec, kind: str) -> Finding:
    ...
    if months is not None and months > cap:
        return _hit(
            ...
            rule,
            rule.tier_hit,
            f"deposit {months}mo exceeds {cap}mo statutory cap",
            data,
        )
...
def refund_rule(text: str, rule: RuleSpec) -> Finding:
    ...
    if not dep:
        return Finding(
            ...
            status="pass",
            tier=1,
            note="no deposit — no refund duty",
        )
    ...
    if m2:
        return _hit(
            text, m2, rule, rule.tier_hit, "refund lacks 1-month itemised statement"
        )
...
def overstay_rule(text: str, rule: RuleSpec) -> Finding:
    ...
    if not m:
        return Finding(
            ...
            status="pass",
            tier=2,
            note="no overstay clause — no risk",
        )
...
def revision_rule(text: str, rule: RuleSpec) -> Finding:
    ...
    if not m:
        return Finding(
            ...
            status="pass",
            tier=2,
            note="no revision clause — no risk",
        )
    ...
    if months is not None and months < 3:
        return _hit(
            text, m, rule, rule.tier_hit, f"revision notice {months}mo < 3mo", data
        )
...
def cap_rule(text: str, rule: RuleSpec) -> Finding:
    ...
    if months is not None and months > 12:
        return _hit(
            text, m, rule, rule.tier_hit, f"cap {months}mo exceeds 12mo norm", data
        )
...
def tds_rule(text: str, rule: RuleSpec) -> Finding:
    ...
    if m:
        return _hit(
            ...
            rule,
            rule.tier_hit,
            "outdated 5% TDS rate (current 2% since 1-Oct-2024)",
        )
